  ...
  return (
    <div className="card p-4 md:p-6 mt-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-lg font-bold text-[var(--color-text-primary)]">Variants ({variants.length})</h2>
          <p className="text-xs text-[var(--color-text-tertiary)] mt-0.5">Edit stock, prices, and status per variant</p>
        </div>
        <button
          onClick={handleSave}
          disabled={saving || changedCount === 0}
          className={cn(
            "btn btn-brand text-sm",
            changedCount === 0 && "opacity-40 pointer-events-none"
          )}
        >
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-[var(--color-border-subtle)] text-xs uppercase text-[var(--color-text-secondary)] bg-[var(--color-surface-base)]">
              ...
              <th className="table-header-cell px-4 py-3 font-semibold text-center w-[120px]">Stock</th>
              ...
              return (
                ...
                >
                  ...
                  <td className="table-cell px-4 py-3">
                    ...
                    {v.sku_variant && (
                      <p className="text-[0.65rem] text-[var(--color-text-tertiary)] mt-0.5 font-mono">
                        {v.sku_variant}
                      </p>
                    )}
                  </td>
                  ...
                  <td className="table-cell px-4 py-3">
                    <div className="flex items-center justify-center gap-1">
                      <button
                        onClick={() => update(v.id, "stock", Math.max(0, e.stock - 1))}
                        disabled={e.stock <= 0}
                        className="w-7 h-7 rounded-lg flex items-center justify-center bg-[var(--color-surface-root)] border border-[var(--color-border-default)] text-[var(--color-text-secondary)] hover:text-red-400 hover:border-red-500/30 disabled:opacity-30 transition-all"
                      >
                        <Minus size={12} />
                      </button>
                      <input
                        type="number"
                        value={e.stock}
                        onChange={(ev) => update(v.id, "stock", Math.max(0, parseInt(ev.target.value) || 0))}
                        min={0}
                        className={cn(
                          "w-14 text-center text-sm font-bold py-1.5 rounded-lg border transition-all",
                          ...
                          e.stock > v.stock && "text-green-400 border-green-500/30",
                          e.stock < v.stock && "text-red-400 border-red-500/30"
                        )}
                      />
                      <button
                        onClick={() => update(v.id, "stock", e.stock + 1)}
                        className="w-7 h-7 rounded-lg flex items-center justify-center bg-[var(--color-surface-root)] border border-[var(--color-border-default)] text-[var(--color-text-secondary)] hover:text-green-400 hover:border-green-500/30 transition-all"
                      >
                        <Plus size={12} />
                      </button>
                    </div>
                    ...
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <input
                        ...
                        onChange={(ev) => {
                          ...
                          update(v.id, "price", val);
                        }}
                        placeholder={formatCurrency(0)}
                        min={0}
                        step={0.01}
                        className={cn(
                          "w-24 text-right text-sm font-medium py-1.5 px-2 rounded-lg border transition-all",
                          "bg-[var(--color-surface-root)] border-[var(--color-border-default)] text-[var(--color-text-primary)]",
                          "focus:border-brand-500/50 focus:outline-none",
                          e.price !== v.price_override && "text-brand-400 border-brand-500/30"
                        )}
                      />
                    </div>
                  </td>
                  ...
                  <td className="table-cell px-4 py-3 text-center">
                    <button
                      onClick={() => update(v.id, "active", !e.active)}
                      className={cn(
                        "px-3 py-1 rounded-full text-[0.65rem] font-semibold uppercase tracking-wider border transition-all",
                        e.active
